...
function FormulaReferenceIntegral({ lang }: { lang: "en" | "zh" }) {
  const formulae = lang === "zh" ? [
    ...
    ["對數形式", "\\ln u=\\log_e u,\\quad \\log u=\\log_{10}u"],
    ["換底公式", "\\log_a u=\\frac{\\ln u}{\\ln a},\\quad \\log u=\\frac{\\ln u}{\\ln 10}"],
    ["對數積分", "\\int \\frac{1}{au+b}\\,du=\\frac{1}{a}\\ln|au+b|+C"],
    ["指數函數", "\\int e^{au+b}\\,du=\\frac{1}{a}e^{au+b}+C"],
    ["一般指數", "a^{ku+b}=e^{(ku+b)\\ln a},\\quad \\int a^{ku+b}\\,du=\\frac{a^{ku+b}}{k\\ln a}+C"],
    ["三角函數", "\\int \\sin(au+b)\\,du=-\\frac{1}{a}\\cos(au+b)+C"],
    ["三角函數", "\\int \\cos(au+b)\\,du=\\frac{1}{a}\\sin(au+b)+C"],
    ["次方擺位", "\\sec^2 u=(\\sec u)^2,\\quad \\sec(u^2)\\text{ 是另一個函數}"],
    ...
    ["Log convention", "\\ln u=\\log_e u,\\quad \\log u=\\log_{10}u"],
    ["Change of base", "\\log_a u=\\frac{\\ln u}{\\ln a},\\quad \\log u=\\frac{\\ln u}{\\ln 10}"],
    ["Logarithmic integral", "\\int \\frac{1}{au+b}\\,du=\\frac{1}{a}\\ln|au+b|+C"],
    ["Exponential", "\\int e^{au+b}\\,du=\\frac{1}{a}e^{au+b}+C"],
    ["General exponential", "a^{ku+b}=e^{(ku+b)\\ln a},\\quad \\int a^{ku+b}\\,du=\\frac{a^{ku+b}}{k\\ln a}+C"],
    ["Trigonometric", "\\int \\sin(au+b)\\,du=-\\frac{1}{a}\\cos(au+b)+C"],
    ["Trigonometric", "\\int \\cos(au+b)\\,du=\\frac{1}{a}\\sin(au+b)+C"],
    ["Power placement", "\\sec^2 u=(\\sec u)^2,\\quad \\sec(u^2)\\text{ is a different function}"],
